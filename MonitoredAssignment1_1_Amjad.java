...
public class MonitoredAssignment1_1_Amjad {
	...
	public static void main(String[] args) {
		...
		boolean xValid = isStringValid(x);	//TODO: if()...
...
//		}
	...
	public static String addition(String op1, String op2) {
		...
		// without a sign
		op1 = stringWithoutSign(op1);	////TODO: It's better to use "op1Modified" instead of op1
		op2 = stringWithoutSign(op2);	////TODO: It's better to use "op2Modified" instead of op2
		...
		for (index = op2.length() - 1; index >= 0; --index) {
			...
			carryIn = carryOut;	//// TODO: Redundant. In this method we use only carryIn.
			...
			if (sum > 9) {
				sum = sum - 10;
				carryOut = 1;	//// TODO: Redundant. In this method we use only carryIn. Here we can write: "carryIn = 1;"
			}
	...
	public static String minus(String op1, String op2) {	//TODO: The name should be "subtraction"
		...
		if (op1.length() < op2.length()) {
			op1 = addZeros(op1, lengthDifference);	//TODO: I would write: "-lengthDifference"
		}
		if (op1.length() > op2.length()) {
			op2 = addZeros(op2, lengthDifference);
		}
...
//			op = index * 0 + op;	//// TODO: Why not "op = 0 + op;" ?
			op = 0 + op;
		}
	...
	private static boolean isStringValid(String x) {	//TODO: isValidString
		...
		if ((x.charAt(0) != '+' && x.charAt(0) != '-') && (x.charAt(0) < '0' && x.charAt(0) > '9')) {
			throw new NumberFormatException("Illegal Input");	//TODO: return false; Exception should be thrown in the highest method.
		}
